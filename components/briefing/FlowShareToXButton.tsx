...
const LABELS = {
  ...
  de: {
    ...
    sub: "Vorschlag für den Post (anpassbar). Das Poster-PNG in X manuell anhängen.",
    postLabel: "Post-Text",
    copy: "Text kopieren",
    ...
    error: "Text konnte nicht erzeugt werden. PNG-Export bleibt möglich.",
...
export function FlowShareToXButton({ language, socialCaptureElementId, payload }: FlowShareToXButtonProps) {
  ...
  const onDownloadSocialPng = useCallback(async () => {
    ...
    try {
      ...
      a.href = dataUrl;
      a.download = `speicherpilot-de-poster-${dateKey}.png`;
      a.click();
    } catch (error) {
